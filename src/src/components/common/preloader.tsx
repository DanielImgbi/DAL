...
export const Preloader = ({ onComplete }: { onComplete: () => void }) => {
  ...
  useEffect(() => {
    ...
    if (digit3) {
      ...
      for (let i = 0; i < 2; i++) {
        for (let j = 0; j < 10; j++) {
          ...
          div.className = "num flex items-center justify-center h-32 md:h-40 text-6xl";
          div.textContent = j.toString();
          digit3.appendChild(div);
        }
      }
      const finalDigit = document.createElement("div");
      finalDigit.className = "num flex items-center justify-center h-32 md:h-40 text-6xl";
      finalDigit.textContent = "0";
      digit3.appendChild(finalDigit);
    }

    const animateDigit = (digit: HTMLElement | null, duration: number, delay: number = 0) => {
      if (!digit) return;
    ...
    const animateImages = () => {
      ...
      const d1 = animateDigit(digit1Ref.current as unknown as HTMLElement, 2, 5);
  ...
  return (
    <div className="preloader-container flex items-center justify-center w-full h-screen bg-black text-white font-anton-sc uppercase fixed overflow-hidden z-[999999]">
      <div className="relative flex items-center justify-center w-full h-full">
        <div className="relative overflow-hidden h-screen w-screen lg:h-[500px] lg:w-100">
        ...
        <div className="absolute bottom-6 right-6 flex flex-col items-center justify-center z-10">
          <div className="flex items-center justify-center">
            <div className="relative h-32 w-10 md:h-40 overflow-hidden">
              <div
                ref={digit1Ref}
                className="absolute w-full digit-1"
              >
                {[0, 1].map((digit) => (
                  <div
                    key={digit}
            ...
            <div className="relative h-32 w-10 md:h-40 overflow-hidden">
              <div
                ref={digit2Ref}
                className="absolute w-full digit-2"
              >
            ...
            <div className="relative h-32 w-10 md:h-40 overflow-hidden">
              <div
                ref={digit3Ref}
                className="absolute w-full digit-3"
              />
            </div>
